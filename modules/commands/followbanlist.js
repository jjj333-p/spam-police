class FollowBanList {
    ...
    async run ({client, roomId, event, mxid, blacklist}, {offset, contentByWords, config}){
        ...
        let joinroom = contentByWords[2+offset]
        ...
        client.resolveRoom(joinroom).then(async joinroomid => {
            ...
            if ( contentByWords[1+offset] == "add" ) {
                ...
                if ( !currentBanlists.some(b => b == joinroomid) ) {
                    ...
                    client.sendHtmlNotice(roomId, ("Not currently subscribed to any banlist with the RoomID <blacklist>" + joinroomid + "</blacklist>."))

                    return

                }

                config.setConfig(roomId, "banlists", currentBanlists.filter(b => b != joinroomid), err => {
                ...
                client.sendHtmlNotice(roomId, ("❌ | Unknown operation <blockquote>" + contentByWords[1+offset] + "</blockquote>, expected <blockquote>add</blockquote> or <blockquote>remove</blackquote>."))
