class Join {
    ...
    async run ({client, roomId, event, mxid, blacklist}, {offset, commandRoom}) {

        //if not run in the command room
        if(roomId != commandRoom){

            client.sendNotice(roomId, "❌ | you must run +join commands in https://matrix.to/#/" + commandRoom + "?via=" + mxid.split(":")[1])
