...
class blacklist {
  ...
  match(roomId) {
    //check if match
    const match = this.blacklistARRAY.find((entry) =>
      entry.split(" ")[0].includes(roomId),
    );
  ...
  async add(roomId, reason) {
    ...
    if (
      this.blacklistARRAY.some((entry) => entry.split(" ")[0].includes(roomId))
    ) {
      //return that as msg
  ...
  async remove(roomId) {
    ...
    this.blacklistARRAY = this.blacklistARRAY.filter(
      (entry) => !entry.split(" ")[0].includes(roomId),
    );
